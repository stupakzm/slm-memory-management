...
def add(db: sqlite3.Connection, rows: list[tuple[dict, list[float]]]) -> None:
    ...
    for c, emb in rows:
        cur.execute(
            "INSERT OR IGNORE INTO chunks(chunk_id,doc_id,ord,char_start,char_end,prefix,text)"
            " VALUES(?,?,?,?,?,?,?)",
...
def search(db: sqlite3.Connection, query_vec: list[float], k: int = 5) -> list[dict]:
    rows = db.execute(
        """SELECT c.chunk_id, c.doc_id, c.text, c.prefix, v.distance
    ...
    return [
        {"chunk_id": r[0], "doc_id": r[1], "text": r[2], "prefix": r[3],
         "distance": r[4], "score": 1.0 - r[4] / 2.0}
        for r in rows
    ]
